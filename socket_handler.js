...
// ------------------------------------------------------------------
// Story 12.8.1 — Public Demo Mode Architecture (WS-entrypoint enforcement)
...
//   - attach ws.role="demo" and ws.demo_jti for downstream guards
// ------------------------------------------------------------------
let demoConfig, getClientIp, hashIp;
try {
  ({ demoConfig } = require("./src/brain/utils/demoConfig"));
} catch (e) {}
try {
  ({ getClientIp, hashIp } = require("./src/db/demoGuards"));
} catch (e) {}
...
// ------------------------------------------------------------------
// Story 12.8.1 (add-on) — Per-demo-token rate limit (jti-based)
// Keeps your tenant limiter intact, adds an extra guard for demo tokens.
// Defaults:
// - 60s window
...
function rateLimitDemoToken(demo_jti) {
  ...
  if (!key) return true; // if no jti, don't block here (demo auth already gates)
...
function tryDecodeJwt(req) {
  ...
  try {
    ...
    // ------------------------------------------------------------------
    // ✅ Story 12.8.1 — Demo token enforcement at WS connect
    // ------------------------------------------------------------------
    ...
    if (isDemo) {
      ...
      if (typeof getClientIp !== "function" || typeof hashIp !== "function") return null;

      const ip = getClientIp(req);
      const expected = hashIp(ip);

      // Must match token’s ip_hash
      if (!decoded.ip_hash || decoded.ip_hash !== expected) return null;

      // Force demo tenant + role
      decoded.tenant_id = cfg.tenantId;
      decoded.partner_id = null;
      decoded.role = "demo";
    }
...
function bindWebSocket(server) {
  ...
  wss.on("connection", (ws, req) => {
    ...
    if (decoded) {
      ...
      // Demo extras (optional)
      if (ws.role === "demo") {
        ws.demo_jti = decoded.demo_jti || decoded.jti || null;
      }
    }
    ...
    ws.on("message", async (raw) => {
      ...
      // ✅ ADDITIONAL demo-token rate limit (jti-based), without changing tenant limiter
      if (ws.role === "demo" && ws.demo_jti) {
        if (!rateLimitDemoToken(ws.demo_jti)) {
          audit({
            type: "ws_demo_token_rate_limited",
            tenant_key: tenantKey,
            session_hash: safeHash(ws.session_id || "no_session"),
            demo_jti_hash: safeHash(ws.demo_jti),
            window_ms: DEMO_TOKEN_RATE_WINDOW_MS,
            max_msgs: getDemoTokenMaxMsgs(),
          });
          try {
            ws.send(
              JSON.stringify({
                ok: false,
                error: "rate_limited",
                message: "Demo rate limit reached. Please slow down.",
              })
            );
          } catch {}
          return;
